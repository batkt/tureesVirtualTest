              ...
              <div className="flex flex-col gap-3 px-3 font-medium py-5 xl:border-t-2">
                <div className="flex flex-col overflow-y-auto" style={{ height: "calc( 100vh - 18rem )" }}>
                  ...
                  <Divider orientation="left">
                    ...
                    {jagsaalt?.map((a) => (
                        <div key={a._id} className="my-2 rounded-md bg-gray-100 hover:shadow-lg dark:bg-gray-700 p-5 py-2">
                          <div className="font-bold mx-auto w-max mb-3">{a.ner}</div>
                          {
                            a.khaalga?.map(b => (
                                <Col span={8}>
                                  <Checkbox
                                      onChange={() => {
                                        setKhaalgaErkh((value) => {
                                          const index = value.findIndex((row) => row === b._id);
                                          if (index !== -1) value.splice(index, 1);
                                          else value.push(b._id);
                                          return [...value];
                                        });
                                      }}
                                      checked={!!khaalgaErkh.find((c) => c === b._id)}>{b.ner}</Checkbox>
                                </Col>
                            ))
                          }
                        </div>
