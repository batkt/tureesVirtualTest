  ...
  return (
    ...
    >
      ...
      <Card className="cardgrid col-span-12 ">
        <div className="hideScroll flex w-full gap-3 overflow-hidden overflow-x-auto border-solid py-3 sm:grid sm:grid-cols-6 sm:p-0 md:gap-4 2xl:grid-cols-12">
          {khyanaltiinDun.map((mur, index) => {
            return (
              <div
                key={index}
                className={`group relative w-[67vw] cursor-pointer overflow-hidden rounded-2xl transition-all duration-300 ease-out hover:-translate-y-2 hover:scale-105 hover:shadow-2xl hover:shadow-gray-300 dark:hover:shadow-gray-800 sm:col-span-12 sm:w-auto lg:col-span-2 ${
                  mur?.utga === shuult?.utga
                    ? `${mur.selectedColor} ${mur.border} border-2`
              ...
              >
                <div
                  className={`absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-10 ${mur.color.replace(
                    "text-",
                    "bg-"
                  )}`}
                />
                ...
                <div className="relative h-full rounded-2xl p-3 sm:p-2.5">
                  <div className="flex h-full flex-col justify-between">
                    ...
                    <div
                      className={`h-0.5 w-0 rounded-full transition-all duration-500 group-hover:w-full ${mur.color.replace(
                        "text-",
                        "bg-"
                      )}`}
                    />
