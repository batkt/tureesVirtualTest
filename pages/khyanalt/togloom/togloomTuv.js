  ...
  ({ data, destroy, confirm, token }, ref) => {
    ...
    React.useImperativeHandle(
      ...
      () => ({
        khadgalya() {
          if (!niitDun) {
            toast.warning("Сунгах хугацаа эсвэл асран хамгаалагчаа нэмнэ үү!");
            setKhugatsaa(false);
            return;
          }
...
function togloom1() {
  ...
  function qrKhevlekh(duusakhTsag, ekhlekhTsag) {
    ...
    if (Number(today) <= Number(duusakhUdur)) {
      ...
      } else toast.warning("Цаг дууссан байна!");
    } else toast.warning("Цаг дууссан байна!");
  }
  ...
  function orlogiinDelegrengui() {
    ...
    modal({
      title: t("Орлогын дэлгэрэнгүй"),
      icon: <FileExcelOutlined />,
      content: (
        <TogloomTulburiinDelgerenguiTailan
          ref={delegrenhuiRef}
          defualtOgnoo={ognoo}
          token={token}
  ...
  useKeyboardTovchlol("F2", () => {
    ...
    tulburRef.current === null &&
      (!!data
        ? tulburTulyu(data)
        : toast.warning("Төлбөр төлөх үйлчлүүлэгч байхгүй байна"));
  });
