...
function cameraVals({ token }) {
  ...
  const cameraParking = () => {
    ...
    if (!!jagsaalt && jagsaalt.length > 1) {
      ...
      if (l === 2 || l === 4) div = 2;
      else if (l === 3 || l === 5 || l === 6 || l === 9) div = 3;
      else if (l === 7 || l === 8 || l === 10) div = 4;
      else div = 6;
      w = "w-1/" + div;
      h = h / div;
    }
        ...
        <div className="flex flex-row flex-wrap">
          {cameraJagsaalt.map((data, index) => {
            return (
              ...
              >
                <div
                  className={`m-1 flex cursor-pointer flex-col justify-between font-bold sm:flex-row ${
                    cameraKharakh === index &&
                    "absolute top-5 ml-5 w-[50%] text-white"
                  }`}
                >
                ...
                <div
                  className={`m-1 rounded-md bg-[url('/notPlay.png')] bg-[length:100%_100%] bg-center ${
                    cameraKharakh === index
                      ? `sm:h-[80vh] sm:w-[80%]`
                      : `sm:h-[${cameraH}px]`
                  }`}
                  style={{ height: cameraKharakh === index ? "80vh" : cameraH }}
                  onClick={(e) => {
                    e.stopPropagation();
                    setCamerKharakh(index);
                  }}
                >
                ...
                <div
                  className={`m-1 mt-1 flex flex-col justify-between font-bold sm:flex-row ${
                    cameraKharakh === index && "absolute bottom-5 w-40"
                  }`}
                >
                  <div className="flex gap-3">
                    <Button
                  ...
                  <div
                    className={`${
                      cameraKharakh === index && "ml-5 text-white"
                    }`}
                  >
                    {data.cameraIP}
                  </div>
