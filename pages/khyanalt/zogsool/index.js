...
function Zogsool({ token }) {
  ...
  const query = useMemo(() => {
      ...
      baseQuery["tuukh.tulbur.turul"] =
        tulbur === "card"
          ...
          : tulbur?.toLowerCase() === "qpay"
            ...
            : tulbur?.toLowerCase() === "ugaalga"
              ? { $regex: "^ugaalga", $options: "i" }
              : tulbur;
    }
                    ...
                    { "tuukh.0.tulbur.0.turul": "khungulult" },
                    {
                      $expr: {
                        $ne: [
                          {
                            $arrayElemAt: [
                              { $arrayElemAt: ["$tuukh.tulbur.dun", 0] },
                              0,
                            ],
                          },
    ...
    if (tootsooKhelber === "2") {
      ...
      console.log(moment(ognoo[0]).format("YYYY-MM-DD 00:00:00"), moment(ognoo[1]).format("YYYY-MM-DD 23:59:59"));
      if (tuluv === 2) {
    ...
    //   //     break;
    //   //   case 2:
    //   //     baseQuery["tuukh.tsagiinTuukh.orsonTsag"] = {
    //   //       $gte: moment(ognoo[0]).format("YYYY-MM-DD 00:00:00"),
    //   //     };
    //   //   case 3:
    //   //     baseQuery["tuukh.tsagiinTuukh.garsanTsag"] = {
    //   //       $gte: moment(ognoo[0]).format("YYYY-MM-DD 00:00:00"),
    //   //       $lte: moment(ognoo[1]).format("YYYY-MM-DD 23:59:59"),
    //   //     };
    //   // }
  ...
  const or = useMemo(() => {
              ...
              $and: [
                {
                  $eq: [
                    {
                      $arrayElemAt: [
                        { $arrayElemAt: ["$tuukh.tulbur.turul", 0] },
                        0,
                      ],
                    },
                    "khungulult",
    ...
    return nemeh;
  }, [ognoo, zogsoolId, shuult, tuluv, tulbur, tootsooKhelber]);
                ...
                .then(({ data }) => {
                  if (data === "Amjilttai") {
                    ...
                    notification.warning({
                      message:
                        "Зөвхөн идэвхтэй машины дугаарыг цэвэрлэх боломжтой!",
                      duration: 2,
                    });
                  }
                })
      ...
      <Card className="col-span-12">
        <div className="grid-cols-3 gap-5 sm:grid xl:flex">
          ...
          >
            ...
            {baiguullaga?._id === "662de8c8919a3695ffe51e36" &&
              ajiltan?.erkh === "Admin" && (
                <div className="flex items-start justify-center gap-1">
                  <Button
                    onClick={ustgakh}
