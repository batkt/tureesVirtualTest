...
function Khyanalt({ token }) {
  ...
  const mashinQuery = useMemo(() => {
    let query = {};
    if (tuluv) {
      query["turul"] = tuluv;
    }
    if (!tuluv || tuluv === "Бүгд") {
      query["turul"] = undefined;
    }
    return query;
  }, [tuluv]);

  const { mashinGaralt, setMashinKhuudaslalt } = useMashin(
    token,
    baiguullaga?._id,
    mashinQuery,
    order
    ...
    if (content !== "") {
      ...
      if (songogdsonKhariltsagch.length > 0)
        songogdsonKhariltsagch.map((a) => {
          var text = msj;
          a.ezemshigchiinNer = a.ezemshigchiinNer || "";
          a.ezemshigchiinUtas = a.ezemshigchiinUtas || "";
          a.dugaar = a.dugaar || "";
          a.turul = a.turul || "";
          a.ekhlekhOgnoo = a.ekhlekhOgnoo || "";
          a.duusakhOgnoo = a.duusakhOgnoo || "";
          for (const [key, value] of Object.entries(a)) {
            text = text?.replace(new RegExp(`<${key}>`, "g"), value);
          }
          msgnuud.push({
            to: a.ezemshigchiinUtas,
            text,
          });
        });
      else {
  ...
  return (
    <Admin
      ...
      onSearch={(search) => setMashinKhuudaslalt((a) => ({ ...a, search }))}
      loading={waiting}
    >
              ...
              <div>
                {a.turul === turul ? (
                  <div
                    ...
                    onClick={() => {
                      if (songogdsonKhariltsagch?.length > 0) zagvarSongokh(a);
                    }}
                  >
        ...
        <div className={`box p-5 xl:block`}>
          <div className="relative w-full text-gray-700   dark:text-gray-300">
            <input
              ...
              placeholder={t("Хайх /Нэр, Регистр, Утас, Машины дугаар/")}
              onChange={({ target }) => {
                clearTimeout(timeout);
                timeout = setTimeout(function () {
                  setMashinKhuudaslalt((a) => ({
                    ...a,
                    search: target.value,
                  }));
                }, 300);
          ...
          <div className=" mt-2  flex cursor-pointer flex-row items-center space-x-4 space-y-2 rounded-md p-2 ">
            <Checkbox
              checked={
                mashinGaralt?.jagsaalt?.length === songogdsonKhariltsagch.length
              }
              onChange={(e) => {
                ...
                {!!mur._id ? (
                  ...
                  >
                    ...
                    <div className="flex w-full items-center justify-between">
                      <div className="text-xs">{mur?.ezemshigchiinNer}</div>
                      <div className="text-xs text-gray-600 dark:text-gray-400 sm:text-sm">
                        <div className="flex w-full justify-end">
                          <div>{mur?.ezemshigchiinUtas}</div>
                        </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3 mr-auto">
                    <div className="text-base font-medium">
                      {khariltsagch?.ezemshigchiinNer}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 sm:text-sm">
                      <div className="flex">
                        <div>{khariltsagch?.ezemshigchiinUtas}</div>
