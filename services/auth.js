...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!!la) setAjiltniiJagsaalt(JSON.parse(la));
  ...
  const auth = useMemo(
    () => ({
        ...
        uilchilgee()
          ...
          .then(({ data, status }) => {
            if (status === 200) {
              if (
                !!data.token &&
                (data?.result?.erkh === "Zasvarchin" ||
                  data?.result?.erkh === "Injener")
              ) {
                const index = ajiltniiJagsaalt.findIndex(
                  (a) => a._id === data?.result?._id
                );
                if (index !== -1) {
                  message.warning("Тухайн ажилтанаар орсон байна");
                  return;
                }
                const { erkh, ner, zurgiinNer, baiguullagiinId, _id } =
                  data.result;
                let a = {
                  token: data.token,
                  erkh,
                ...
                localStorage.setItem(
                  "ajiltniiJagsaalt",
                  JSON.stringify(ajiltniiJagsaalt)
                );
      ...
      ajiltanKhasya: async (khereglech) => {
        if (ajiltniiJagsaalt.length === 1) return;
        const index = ajiltniiJagsaalt.findIndex(
          (a) => a._id === khereglech._id
        );
