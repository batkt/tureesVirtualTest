...
const ShineDugaarKeyboard = ({
  ...
  const handleButtonClick = (num, shineTurul) => {
    if (shineTurul) {
      if (num !== "Butsakh") {
        if (dugaar.length < 10) {
          setDugaar((prevDugaar) => normalize(prevDugaar + num));
        }
      } else {
        if (dugaar.length > 0) {
          setDugaar((prevDugaar) => normalize(prevDugaar.slice(0, -1)));
        }
      }
    } else {
      ...
      if (khoosonIndex !== -1) {
        if (num !== "Butsakh") {
          ...
          setDugaar(normalize(shineDugaar.join("")));
          if (khoosonIndex < 3) {
    ...
    <div className="flex flex-col items-center justify-between gap-12">
      ...
      {(!shineTurul || eBarimtTurul !== "khuviKhun") && (
        <div className="flex w-full gap-5 ">
          <div className="grid grid-cols-3 place-items-center gap-2 text-5xl font-bold md:gap-[4px]">
            {numbers.map((num, index) => (
              <div
                key={index}
                className={`col-span-1 flex items-center justify-center rounded-xl ${
                  num === 0
                    ? " col-span-3 h-[130px] w-full !bg-[#414143] md:h-[90px] "
          ...
          <div className="grid grid-cols-[repeat(9,minmax(0,1fr))] place-items-center gap-2 text-5xl font-bold md:gap-[4px]">
...
export default ShineDugaarKeyboard;
