        ...
        <div className="dark:text-gray-200">
          ...
          <div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Төлөх огноо")}:</p>{" "}
              <p>
                {moment(ugugdul.daraagiinTulukhOgnoo).format(
                  "YYYY-MM-DD, hh:mm"
                )}
              </p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Регистр")}:</p>{" "}
              <p>{ugugdul.register}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Талбай")}:</p>{" "}
              <p>{talbainDugaar}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Утас")}:</p>{" "}
              <p>{formatNumber(utas)}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Үлдэгдэл")}:</p>{" "}
              <p>{ugugdul.uldegdel}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Сарын түрээс")}:</p>{" "}
              <p>{formatNumber(sariinTurees)}₮</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Талбайн үнэ")}:</p>{" "}
              <p>{formatNumber(ugugdul.talbainNiitUne)}₮</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Давхар")}:</p>{" "}
              <p>{ugugdul.davkhar}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Түрээслэгч")}:</p> <p>{ner}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Гэрээний огноо")}:</p>{" "}
              <p>{moment(gereeniiOgnoo).format("YYYY-MM-DD, hh:mm")}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Алдангийн үлдэгдэл")}:</p>{" "}
              <p>{formatNumber(ugugdul.aldangiinUldegdel)}</p>
            </div>
            {tileProps?.turul === "voucher" && (
              <div className="flex justify-between border-b p-1">
                <p className="font-medium ">{t("Ваучерын дүн")}:</p>{" "}
                <p>{formatNumber(ugugdul.voucherDun)}</p>
              </div>
            )}
            {tileProps?.turul === "khungulult" && (
              <div className="flex justify-between border-b p-1">
                <p className="font-medium ">{t("Хөнгөлөлт")}:</p>{" "}
                <p>{formatNumber(ugugdul.khungulult)}</p>
              </div>
            )}
            {tileProps?.turul === "eneSardTulsun" && (
              <div className="flex justify-between border-b p-1">
                <p className="font-medium ">{t("Төлсөн дүн")}:</p>{" "}
                <p>{formatNumber(ugugdul.tulsunDun)}</p>
              </div>
            )}
            {tileProps?.turul === "eneSardTulukh" && (
              <div className="flex justify-between border-b p-1">
                <p className="font-medium ">{t("Төлөвлөгөөт")}:</p>{" "}
                <p>{formatNumber(ugugdul.tuluvluguut)}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex w-full flex-row items-center justify-between">
            ...
            >
              <Progress
                type="circle"
                percent={
                  !khuvi && 1 > khuvi ? khuvi?.toFixed(1) : khuvi?.toFixed(0)
                }
                width={22}
