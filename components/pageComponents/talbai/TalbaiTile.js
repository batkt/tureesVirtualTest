      ...
      <div className="space-y-5 ">
        <div className="dark:text-gray-200">
          ...
          <div className="dark:text-gray-200">
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Бүртгэсэн Огноо")}:</p>{" "}
              <p>{moment(ugugdul.createdAt).format("YYYY-MM-DD hh:mm")}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Давхар")}:</p> <p>{davkhar}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Талбай/м2/")}:</p>{" "}
              <p>{talbainKhemjee}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Нийт үнэ/₮/")}:</p>{" "}
              <p>{formatNumber(talbainNiitUne)}</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Зардал")}:</p>{" "}
              <p>{ugugdul.niitAshiglaltiinZardal}</p>
            </div>
            <div className="flex justify-between border-b p-1">
              <p className="font-medium ">{t("Төлбөр")}:</p>{" "}
              <p>{formatNumber(ugugdul.tureesiinTulbur)}₮</p>
            </div>
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Тайлбар")}:</p>{" "}
              <p>{ugugdul.tailbar || "хоосон"}</p>
            </div>
            <div className="flex justify-between border-b p-1">
            ...
            <div className="flex justify-between border-b bg-green-500 bg-opacity-10 p-1">
              <p className="font-medium ">{t("Талбайн төрөл")}:</p>{" "}
              <p>{ugugdul.niitiinTalbaiEsekh ? "Нийтийн талбай" : "Үндсэн"}</p>
            </div>
          </div>
        </div>
        <div className="flex w-full justify-between gap-2">
          ...
          <Popconfirm
            ...
            cancelText={t("Үгүй")}
            onConfirm={() => {
              tileProps?.talbaiUstgay(ugugdul), destroy();
            }}
          >
            <Button
              className="w-full"
